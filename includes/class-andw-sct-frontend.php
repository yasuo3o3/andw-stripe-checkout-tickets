...
class Andw_Sct_Frontend {
    ...
    private function enqueue_assets( array $settings ) : void {
        ...
        $data = [
            ...
            'consent'    => [
                ...
                'message' => __( '�w���O�ɓ��Ӄ`�F�b�N�����Ă��������B', 'andw-sct' ),
            ],
            'messages'   => [
                'genericError'  => __( '�`�F�b�N�A�E�g�̊J�n�Ɏ��s���܂����B���Ԃ�u���čĎ��s���Ă��������B', 'andw-sct' ),
                'loginRequired' => __( '���O�C�����K�v�ł��B', 'andw-sct' ),
                'processing'    => __( '������...', 'andw-sct' ),
            ],
        ];
    ...
    public function render_checkout_button( $atts = [] ) : string {
        ...
        if ( '' === $sku ) {
            return '<p>' . esc_html__( 'SKU���w�肵�Ă��������B', 'andw-sct' ) . '</p>';
        }

        $settings = Andw_Sct_Settings::get_settings();
        $price_id = $this->checkout->get_price_id( $sku );
        if ( ! $price_id ) {
            return '<p>' . esc_html__( '�w�肳�ꂽSKU�ɑΉ�����Price ID���ݒ肳��Ă��܂���B', 'andw-sct' ) . '</p>';
        }
        ...
        if ( $require_login && ! is_user_logged_in() ) {
            ...
            return sprintf(
                ...
                wp_kses_post( sprintf( __( '�w���ɂ̓��O�C�����K�v�ł��B<a href="%s">���O�C���͂�����</a>�B', 'andw-sct' ), esc_url( $login_url ) ) )
            );
        }

        $quantity      = max( 1, absint( $atts['qty'] ) );
        $label         = sanitize_text_field( $atts['label'] );
        $button_label  = $label ? $label : __( '�`�P�b�g���w��', 'andw-sct' );
    ...
    public function render_buttons_group( $atts = [] ) : string {
        ...
        if ( empty( $group ) ) {
            return '<p>' . esc_html__( '�w�肳�ꂽ�O���[�v�Ƀ{�^�����o�^����Ă��܂���B', 'andw-sct' ) . '</p>';
        }
        ...
        foreach ( $group as $button ) {
            ...
            $html        .= $this->render_checkout_button(
                [
                    'sku'           => $button['sku'] ?? '',
                    'qty'           => $button['qty'] ?? 1,
                    'label'         => $label ?: __( '�`�P�b�g���w��', 'andw-sct' ),
    ...
    public function render_thanks( $atts = [], $content = '' ) : string {
        ...
        if ( '' === $session_id ) {
            return '<p>' . esc_html__( '�Z�b�V����ID���w�肳��Ă��܂���B', 'andw-sct' ) . '</p>';
        }

        $summary = $this->checkout->get_session_summary( $session_id );
        if ( is_wp_error( $summary ) ) {
            return '<p>' . esc_html__( '�w�������擾�ł��܂���ł����BStripe����̃��[�����m�F���Ă��������B', 'andw-sct' ) . '</p>';
        }
        ...
        <div class="andw-sct-thanks">
            <h2><?php esc_html_e( '���w�����肪�Ƃ��������܂�', 'andw-sct' ); ?></h2>
            <p><?php esc_html_e( '�w�����e���ȉ��ɕ\�����܂��B', 'andw-sct' ); ?></p>
            <ul class="andw-sct-thanks__list">
                <?php foreach ( $line_items as $item ) : ?>
                    <li>
                        <span class="andw-sct-thanks__item-name"><?php echo esc_html( $item['description'] ); ?></span>
                        <span class="andw-sct-thanks__item-qty"><?php echo esc_html( sprintf( __( '%d��', 'andw-sct' ), $item['quantity'] ) ); ?></span>
                        <span class="andw-sct-thanks__item-amount"><?php echo esc_html( $item['amount_display'] ); ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
            <p class="andw-sct-thanks__total">
                <?php echo esc_html( sprintf( __( '���v: %s', 'andw-sct' ), $summary['amount_total_display'] ) ); ?>
            </p>
            <?php if ( ! empty( $session['receipt_url'] ) ) : ?>
                <p><a class="andw-sct-button-link" href="<?php echo esc_url( $session['receipt_url'] ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Stripe�̎������J��', 'andw-sct' ); ?></a></p>
            <?php endif; ?>
            <?php if ( ! empty( $settings['meeting_form_url'] ) ) : ?>
                <p><a class="andw-sct-button-link" href="<?php echo esc_url( $settings['meeting_form_url'] ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( '�ł����킹�t�H�[����', 'andw-sct' ); ?></a></p>
            <?php endif; ?>
            <p><a class="andw-sct-button-link" href="#andw-sct-register"><?php esc_html_e( '���[�U�[�o�^�i30�b�Ŋ����j', 'andw-sct' ); ?></a></p>
